...
that will listen on all interfaces for both IPv4 and IPv6.
...
If the operating system supports Dual-Stack sockets with ipv => "*"
(default), then both IPv4 and IPv6 will be bound on the same IPv6 port.
If you receive an IPv4 connection on a bound IPv6 port, the textual
representation of the peer's IPv4 address will typically be in a
form of an IPv4-mapped IPv6 addresses, e.g. "::ffff:127.0.0.1" .
If this behavior is not desired, use ipv => "v4v6" instead of "*",
or use distinct ports, e.g.,

  port => "20203", ipv =>"IPv4v6", # v4v6

  port => "20203/IPv4/IPv6", # Stacked IP Protocols

  port => ["20203/IPv4","20203/IPv6"], # separate ports

  port => ["*:20203/IPv4","*:20203/IPv6"], # distinct ipv

  port => ["0.0.0.0:20203","[::]:20203"], # explicit listen hosts
...
Thanks to Kristoffer Mullerhoj for fixing UDP on Multiplex.
...
Thanks to Grok 4.1 xAI for providing patches for various fixes.
