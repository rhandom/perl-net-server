...
=head1 FEATURES
    ...
    * Full IPv6 support (via IPv6 style addr, or via ipv => "IPv6")
...
specification can be passed as part of the hostname.
    ...
    host => 'localhost',  # addresses matched by localhost (default IPv4)

    ipv  => 6,
    host => 'localhost',  # addresses matched by localhost (IPv6)

    ipv  => '*',
    host => 'localhost',  # addresses matched by localhost (any IPv6 or IPv4)
...
IPv6 is now available under Net::Server though it is not enabled as a
default parameter.  It must be requested explictly by passing either a
value of 6, or IPv6, or a "*" or IPv* to the ipv configuration
...
is left as an easy option to set and enable - and Net::Server has gone
to great lengths to make it easy to enable (it can specified directly,
as part of port, part of host, part of protocol, or you can set
$ENV{IPV}).  Additionally, if a hostname is used, ipv can be set to
"*".  If IPv6 is enabled on the box and there is one or more IPv6 or
IPv4 interfaces associated with that name, they will all be bound.
...
Essentially, the ipv parameter was chosen because it does not conflict
with any other existing usage, it is very similar to ipv4 or ipv6, it
allows for user code to not need to know about Socket::AF_INET or
