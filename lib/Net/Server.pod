...
C<Net::Server> combines the good properties from C<Net::Daemon>
(0.34), C<NetServer::Generic> (1.03), and C<Net::FTPServer> (1.0), and
...
be combined with the SSLEAY proto to provide an HTTPS Daemon.  See
L<Net::Server::HTTP>.
...
C<syslog_facility> may also be defined.  If a C<log_file> is given or
if C<setsid> is set, STDIN and STDOUT will automatically be opened to
...
designation (eg. "http"), whee I<ipv> represents the IP protocol version
(IPv4 or IPv6) and where I<proto> represents the protocol to be
...
strings:
    ...
    localhost:20203/tcp/IPv4         # host, port, protocol and family
    localhost, 20203, tcp, IPv4      # same
    localhost | 20203 | tcp | IPv4   # same
    localhost:20203/IPv4/IPv6        # bind localhost on IPv4 and 6
    localhost:20203/IPv*             # bind configured interfaces for IPv4 or 6
...
the port may be passed as a hashref or array hashrefs of information:
    ...
    port => [{
        host  => '*',
        port  => '20203',
        ipv   => 4,     # IPv4 only (default)
        proto => 'tcp', # (default)
    }, {
        host  => 'localhost',
        port  => '20204',
        ipv   => '*',      # all IPv4 and IPv6 interfaces tied to localhost
        proto => 'ssleay', # Using SSL
    }],
...
2.0, Net::Server supports tcp, udp, and unix, unixdgram, and ssleay.
Other types will need to be added later (or custom modules extending
the Net::Server::Proto class may be used).  Configuration passed to
new or run may be either a scalar containing a single proto or an
arrayref of protos - if the protos array is shorter than the ports
array, the last proto entry will be used to augment the protos arrary
to the size of the ports array.
...
when ipv is set to 6 will result in an error.  The following port
string specifications would use IPv6.

    localhost:20203:IPv6  # will use IPv6
    [::1]:20203           # defaults to IPv6
...
In many proposed Net::Server solutions, IPv6 was enabled by default,
however doing so brings immediate security implications, as well as
potential speed bottlenecks, and all of this in addition to hostname
ambiguity (such a problem exists in IO::Socket::SSL).  Therefore, it
is left as an easy option to set and enable.  Additionally, if a
hostname is used, ipv can be set to "*".  If IPv6 is enabled on the
box and there is one or more IPv6 or IPv4 interfaces associated with
that name, they will all be bound.

For example, on my local box the following command results in the
following output:

    perl -e 'use base qw(Net::Server); main->run(host => "localhost", ipv => "*")'

    2012/05/24-16:44:29 main (type Net::Server) starting! pid(31904)
...
bind ports rather than depending upon "*" to resolve them for me.
    ...
    2012/05/24-16:48:36 main (type Net::Server) starting! pid(409)
    Binding to TCP port 20203 on host localhost with IPv4
    Binding to TCP port 20203 on host localhost with IPv6

There is a special case of using host => "*" as well as ipv => "*".  In
this case, if IPv6 is available, it will answer (at least on linux) for
both IPv6 as well as IPv4.  In this special case ipv => "*" will only
return one resolution:

    perl -e 'use base qw(Net::Server); main->run(host => "*", ipv => "*")'

    2012/05/24-16:54:21 main (type Net::Server) starting! pid(2051)
    Resolved [*]:20203 to [0.0.0.0]:20203, IPv4
    Resolved [*]:20203 to [::]:20203, IPv6
    Removing doubly bound port [0.0.0.0] IPv4 because it should be handled by [::] IPv6
    Binding to TCP port 20203 on host :: with IPv6

An INET socket can only accept IPv4 connections. An INET6 socket
accepts IPv6 connections, but may also accept IPv4 connections
depending on OS and its settings. For example, on FreeBSD systems
setting a sysctl net.inet6.ip6.v6only to 0 will allow IPv4 connections
to an INET6 socket.  If this is the case, specifying "::" as a binding
address instead of a "*" might be desired to reduce the number of
sockets needed and remove any of the associated overhead. Note that a
textual representation of a peer's IPv4 address as connected to an
INET6 socket will typically be in a form of an IPv4-mapped IPv6
...
for STDIN and STDOUT.  This interface allows SSLEAY to work.  It also
